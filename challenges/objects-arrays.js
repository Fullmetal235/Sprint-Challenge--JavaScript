...
const contactInfo = [];
//for (let i = 0; i < graduates.length; i++){
 // contactInfo[i] = graduates[i]["first_name", "email" ];
//}
...
let lowerCase = [];
zooAnimals.forEach(function(object){
  let name = object.animal_name;
  lowerCase.push(name)})

const lower = function(string){
  return string.toLowerCase();
}
lowerCase = lowerCase.map(lower)
...
*/
let lowerPopulation = [];
const lowSized = function(object){
  return object.population < 5;
}
lowerPopulation = zooAnimals.filter(lowSized)
